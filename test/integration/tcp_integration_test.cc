...
void Client(std::uint16_t port, std::uint64_t id, std::atomic<std::size_t>* completed) {
  ...
  assert(send(socket, frame.data(), static_cast<int>(frame.size()), 0) ==
         static_cast<int>(frame.size()));
  std::string response(18, '\0');
  std::size_t received = 0;
  while (received < response.size()) {
    const auto count = recv(socket, response.data() + received, response.size() - received, 0);
    assert(count > 0);
...
int main() {
  ...
  for (std::uint64_t id = 1; id <= 1000; ++id)
    clients.emplace_back(Client, server.port(), id, &completed);
