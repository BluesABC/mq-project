...
std::uint32_t Get32(const std::string& value, std::size_t position) { std::uint32_t result = 0; for (int i = 0; i < 4; ++i) result = (result << 8) | static_cast<unsigned char>(value[position + i]); return result; }

mq::protocol::Request CreateTopic() { mq::protocol::Request request; request.command = mq::protocol::Command::kCreateTopic; request.topic = "replication"; Put32(&request.payload, 1); return request; }
mq::protocol::Request Produce() { mq::protocol::Request request; request.command = mq::protocol::Command::kProduce; request.topic = "replication"; Put32(&request.payload, 0xFFFFFFFFu); Put16(&request.payload, 1); request.payload.push_back('k'); Put32(&request.payload, 1); request.payload.push_back('v'); return request; }
mq::protocol::Request ProduceAckAll() { auto request = Produce(); request.flags = mq::protocol::kAckAll; return request; }
}
...
int main() {
  ...
  const auto produced = broker.Handle(Produce()); assert(produced.status == mq::protocol::Status::kOk);
  mq::network::TcpServer server("127.0.0.1", 0, 2, [&broker](const auto& request) { return broker.Handle(request); });
  assert(server.Start());
  ...
  mq::network::TcpServer follower_server("127.0.0.1", 0, 2, [&follower](const auto& request) { return follower.Handle(request); });
  assert(follower_server.Start());
  mq::server::Broker leader(leader_root); assert(leader.Open());
