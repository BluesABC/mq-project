...
std::string TopicCode(const std::string& topic) {
  ...
  for (const unsigned char byte : topic) {
    encoded.push_back(kHex[byte >> 4]); encoded.push_back(kHex[byte & 0x0F]);
  }
  return encoded;
}
...
StorageEngine::~StorageEngine() {
  { std::lock_guard<std::mutex> lock(mutex_); stop_cleaner_ = true; }
...
StorageEngine::Partition* StorageEngine::GetPartition(const std::string& topic,
  ...
  item->topic = topic; item->number = partition;
  ...
  if (ec) { if (error) *error = ec.message(); return nullptr; }
...
bool StorageEngine::Recover(Partition* partition, std::string* error) const {
  ...
  for (const auto& path : paths) {
    ...
    segment->path = path;
    try { segment->number = std::stoull(path.stem().string()); } catch (...) { continue; }
    segment->index_path = path; segment->index_path.replace_extension(".index");
    segment->file.open(path, std::ios::in | std::ios::out | std::ios::binary | std::ios::app);
    if (!segment->file) { if (error) *error = "cannot open WAL segment"; return false; }
    ...
    while (input) {
      ...
      const std::string h(header, sizeof(header)); const auto crc = Get32(h, 0); const auto len = Get32(h, 4);
      ...
      Message msg; msg.offset = Get64(body, 0); msg.timestamp_ms = static_cast<std::int64_t>(Get64(body, 8));
      msg.key.assign(body, 18, key_len); msg.value.assign(body, 22 + key_len, value_len);
      if (msg.offset != partition->next_offset) break;
      if (msg.offset % config_.index_interval == 0) { std::string item; Put64(&item, msg.offset); Put64(&item, valid_bytes); index.write(item.data(), 16); }
      segment->messages.push_back(std::move(msg)); ++partition->next_offset;
...
bool StorageEngine::Append(const std::string& topic, std::uint32_t partition, std::string key,
  ...
  if (!IsValidTopicName(topic) || message == nullptr || key.size() > 65535 || value.empty() || value.size() > kMaxMessageBytes) {
    if (error) *error = "invalid message"; return false;
  }
  std::lock_guard<std::mutex> lock(mutex_);
  if (!opened_) {
    std::error_code ec; std::filesystem::create_directories(data_dir_ / "queues", ec);
    if (ec) { if (error) *error = ec.message(); return false; }
    opened_ = true;
  }
  auto* target = GetPartition(topic, partition, error); if (!target) return false;
  auto& active = target->segments.back();
  Message next; next.offset = target->next_offset;
  next.timestamp_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
  next.key = std::move(key); next.value = std::move(value);
  ...
  if (active->size != 0 && active->size + record.size() > config_.segment_size_bytes) {
    auto segment = std::make_unique<Partition::Segment>(); segment->number = active->number + 1;
    segment->path = SegmentPath(target->directory, segment->number); segment->index_path = segment->path; segment->index_path.replace_extension(".index");
  ...
  auto& current = target->segments.back(); const auto position = current->size;
  current->file.clear(); current->file.seekp(0, std::ios::end); current->file.write(record.data(), static_cast<std::streamsize>(record.size()));
  if (!current->file) { if (error) *error = "WAL write failed"; return false; }
  if (next.offset % config_.index_interval == 0) { std::ofstream index(current->index_path, std::ios::binary | std::ios::app); std::string item; Put64(&item, next.offset); Put64(&item, position); index.write(item.data(), 16); }
  current->size += record.size(); current->last_timestamp_ms = next.timestamp_ms; current->messages.push_back(next); ++target->next_offset;
  current->file.flush();
  const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - target->last_sync).count();
  const bool sync = config_.fsync_policy == FsyncPolicy::kPerMessage || (config_.fsync_policy == FsyncPolicy::kInterval && elapsed >= config_.fsync_interval_ms);
  if (sync && (!SyncPath(current->path) || !SyncPath(current->index_path))) { if (error) *error = "WAL fsync failed"; return false; }
...
bool StorageEngine::Read(const std::string& topic, std::uint32_t partition, std::uint64_t start_offset,
  ...
  std::lock_guard<std::mutex> lock(mutex_); auto* target = GetPartition(topic, partition, error); if (!target) return false;
  messages->clear(); std::uint64_t bytes = 0;
  for (const auto& segment : target->segments) for (const auto& item : segment->messages) {
    ...
    messages->push_back(item); bytes += size;
  }
  return true;
}
...
bool StorageEngine::Flush(std::string* error) {
  std::lock_guard<std::mutex> lock(mutex_);
  for (const auto& partition : partitions_) for (const auto& segment : partition->segments) {
    segment->file.flush();
    if (!segment->file || !SyncPath(segment->path) || !SyncPath(segment->index_path)) { if (error) *error = "WAL flush failed"; return false; }
  }
  return true;
}
...
bool StorageEngine::CleanupExpiredSegments(std::string* error) {
  ...
  for (const auto& partition : partitions_) {
    std::uint64_t total = 0; for (const auto& segment : partition->segments) total += segment->size;
    while (partition->segments.size() > 1) {
      const auto& oldest = partition->segments.front();
      const bool expired = config_.retention_ms != 0 && oldest->last_timestamp_ms != 0 && now - oldest->last_timestamp_ms >= static_cast<std::int64_t>(config_.retention_ms);
      const bool oversized = config_.retention_bytes != 0 && total > config_.retention_bytes;
      if (!expired && !oversized) break;
      total -= oldest->size; oldest->file.close(); std::error_code ec;
      std::filesystem::remove(oldest->path, ec); if (!ec) std::filesystem::remove(oldest->index_path, ec);
      if (ec) { if (error) *error = ec.message(); return false; }
...
void StorageEngine::CleanerLoop() {
  ...
  while (true) {
    ...
    if (cleaner_cv_.wait_for(lock, std::chrono::milliseconds(config_.cleaner_interval_ms),
                             [this] { return stop_cleaner_; })) return;
    lock.unlock();
