...
bool Parse(int argc, char** argv, Options* options) {
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    if (key == "--topic")
      ...
      options->group = value;
    else if (key == "--port" && Number(value, &number) && number <= 65535)
      options->port = static_cast<std::uint16_t>(number);
    else if (key == "--messages" && Number(value, &number))
      options->messages = number;
    else if (key == "--size" && Number(value, &number))
      options->size = number;
    else if (key == "--connections" && Number(value, &number) && number > 0 && number <= 10000)
      options->connections = static_cast<std::uint32_t>(number);
    else if (key == "--batch" && Number(value, &number) && number > 0 && number <= 10000)
      options->batch = static_cast<std::uint32_t>(number);
    else if (key == "--partitions" && Number(value, &number) && number > 0 && number <= 1024)
...
bool ResolveTopicPartitions(const Options& options, std::uint32_t* partitions, std::string* error) {
  ...
  if (!producer.connect(options.host, options.port)) {
    if (error != nullptr)
      *error = producer.lastError().empty() ? "connect failed" : producer.lastError();
    return false;
  }
  std::vector<mq::client::TopicInfo> topics;
  if (!producer.listTopics(&topics)) {
    if (error != nullptr)
      *error = producer.lastError().empty() ? "list topics failed" : producer.lastError();
    return false;
  }
...
bool TryCountMessage(std::atomic<std::uint64_t>* completed, std::uint64_t limit) {
  ...
  while (current < limit &&
         !completed->compare_exchange_weak(current, current + 1, std::memory_order_relaxed,
...
std::uint32_t EffectiveBatch(const Options& options) {
  ...
  return static_cast<std::uint32_t>(
      std::max<std::uint64_t>(1, std::min<std::uint64_t>(options.batch, maximum)));
}
void Report(const Options& options, std::uint64_t count,
            std::chrono::steady_clock::time_point start, std::vector<double> latencies) {
  std::sort(latencies.begin(), latencies.end());
  const double seconds =
      std::chrono::duration<double>(std::chrono::steady_clock::now() - start).count();
  auto percentile = [&](double p) {
    const auto index = static_cast<std::size_t>(p * latencies.size());
    return latencies[index < latencies.size() ? index : latencies.size() - 1];
  };
  std::cout << kBenchVersion << " mode=" << options.mode << " messages=" << count
            << " size=" << options.size << " connections=" << options.connections
            << " batch=" << options.batch << " partitions=" << options.partitions
            << " total_seconds=" << seconds << " TPS=" << count / seconds << " avg_latency_us="
            << std::accumulate(latencies.begin(), latencies.end(), 0.0) / latencies.size()
            << " p50_us=" << percentile(.50) << " p99_us=" << percentile(.99)
            << " p999_us=" << percentile(.999) << '\n';
}
}  // namespace
...
int main(int argc, char** argv) {
  ...
  Options options;
  if (!Parse(argc, argv, &options) || (options.mode != "produce" && options.mode != "consume")) {
    std::cerr << "usage: mq_bench produce|consume --topic test --messages N --size bytes "
                 "[--connections N] [--batch N] [--partitions N]\n";
    return 2;
  }
  const auto effective_batch = EffectiveBatch(options);
  if (effective_batch == 0) {
    std::cerr << "message size exceeds protocol payload limit\n";
    return 2;
  }
  ...
  std::atomic<std::uint64_t> next_message{0};
  ...
  try {
    if (options.mode == "produce") {
      ...
      if (!setup.connect(options.host, options.port)) {
        std::cerr << "connect failed: " << setup.lastError() << '\n';
        return 1;
      }
      ...
      for (std::uint32_t connection = 0; connection < options.connections; ++connection)
        workers.emplace_back([&, connection] {
          try {
            mq::client::MqProducer producer;
            producer.setProducerId(producer_id_base + connection);
            if (!producer.connect(options.host, options.port)) {
              failed = true;
              return;
            }
            ...
            while (!failed.load()) {
              ...
              const auto current = static_cast<std::size_t>(
                  std::min<std::uint64_t>(options.batch, options.messages - begin_message));
              std::vector<mq::client::ProducerMessage> batch;
              batch.reserve(current);
              for (std::size_t index = 0; index < current; ++index)
                batch.push_back({"bench-" + std::to_string(connection) + "-" +
                                     std::to_string(begin_message + index),
                                 std::string(options.size, 'x')});
              const auto begin = std::chrono::steady_clock::now();
              if (!producer.produceBatch(options.topic, batch)) {
                failed = true;
                break;
              }
              {
      ...
      for (std::uint32_t connection = 0; connection < worker_count; ++connection)
        workers.emplace_back([&, connection] {
          try {
            ...
            if (!consumer.subscribe(options.topic, options.group + "_" + std::to_string(connection),
                                    connection)) {
            ...
            while (completed.load() < options.messages && !failed.load()) {
              ...
              if (TryCountMessage(&completed, options.messages)) {
                std::lock_guard lock(latency_mutex);
                latencies.push_back(std::chrono::duration<double, std::micro>(
                                        std::chrono::steady_clock::now() - begin)
                                        .count());
  ...
  const auto total_completed =
      options.mode == "produce"
          ? std::accumulate(producer_completed.begin(), producer_completed.end(), std::uint64_t{0})
          : completed.load();
  if (failed || total_completed != options.messages) {
    std::cerr << "benchmark failed: completed=" << total_completed
              << " expected=" << options.messages << " workers=" << options.connections << '\n';
    if (!worker_error.empty()) std::cerr << worker_error << '\n';
    for (std::size_t i = 0; i < producer_completed.size(); ++i)
      std::cerr << "worker[" << i << "]=" << producer_completed[i] << '\n';
    return 1;
  }
  Report(options, total_completed, start, std::move(latencies));
  return 0;
}
